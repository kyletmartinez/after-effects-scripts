...
gulp.task("build", async () => {
    ...
    for (const category of categories) {
        ...
        try {
            const customizedTemplate = categoryTemplate
                .replace(/{{categoryName}}/g, category)
                .replace(/{{categoryEmoji}}/g, categoryEmojis[category] || "");
    ...
    try {
        const categoryList = categories
            .map(cat => `- ${categoryEmojis[cat] || ""} [${cat}](/${cat})`)
            .join("\n");
        const categoryListRegex = /{{#each categories}}[\s\S]*?{{\/each}}/g;
        const customizedTemplate = rootTemplate.replace(categoryListRegex, categoryList);
                ...
                if (file.isBuffer()) {
                    ...
                    if (match && match[1]) {
                        ...
                        if (fileNameStripped !== docName) {
                            const error = new Error(`Mismatch in ${fileName}.jsx`);
                            return cb(error);
                        }
                    }
                }
